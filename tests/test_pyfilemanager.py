...
def test_add_include_list(tmp_path_factory):
    ...
    fm = FileManager(tmp_path_factory.getbasetemp())
    fm.add(
        "sony42", "*Camera.avi", include=["sony", "42"]
...
def test_add_exclude_str(tmp_path_factory):
    ...
    assert _relative_paths(fm["panasonic"]) == {
        "panasonic/151Camera.avi",
        "panasonic/143Camera.avi",
    }
...
def test_getitem(tmp_path_factory):
    ...
    assert _relative_paths(fm["*notes?.txt"]) == {
        "notes/notes1.txt",
        "notes/notes2.txt",
    }
    ...
    assert _relative_paths(fm["143Camera"]) == {
        "panasonic/143Camera.avi",
        "sony/143Camera.avi",
    }
    ...
    assert _relative_paths(fm["20"]) == {
        "panasonic2/201Camera.avi",
        "panasonic2/202.mp4",
    }
...
def test_report(tmp_path_factory, capsys):
    ...
    assert [" ".join(x.split(" ")[:2]) for x in captured.out.splitlines()] == [
        "2 canon",
        "1 sony42",
        "8 videos",
        "2 panasonic",
...
def test_find(tmp_path_factory):
    ...
    assert _relative_paths(pyfilemanager.find("notes1.txt", path=path)) == {
        "notes/notes1.txt"
    }
    assert (
        _relative_paths(pyfilemanager.find("abc.def")) == set()
...
def test_get_file_sizes(tmp_path_factory):
    ...
    assert (
        list(pyfilemanager.get_file_sizes(str(fname)).values())[0] == 0
    )  # testing when file_list is a string
